...
// UNIT TEST FOR FMULSU INSTRUCTION
...
void testFMULSU (sbyte regD, sbyte regR, sbyte initSreg) {
    sbyte sreg = initSreg;
    sbyte result;

    sword16 outData = ((__int8_t)regD) * regR;
    sreg &= 0xFC;
...
int main(int argc, char *argv[])
{
    ...
    testFMULSU(0xC0,0x40,0x00); //(-0,5)*0,5 = -0,25 (1,75 in two complement)
    printf("DataL: %X\n", output.outL);
    printf("DataH: %X\n", output.outH);
    ...
    testFMULSU(0x80,0x80,0x00); //-1*128 = -1 (two complement overflow)
    printf("DataL: %X\n", output.outL);
    printf("DataH: %X\n", output.outH);
    assert(output.outL == 0x00);
    ...
    testFMULSU(0x40,0x40,0x00); //0,5*0,5 = 0,25
    printf("DataL: %X\n", output.outL);
    printf("DataH: %X\n", output.outH);
    assert(output.outL == 0x00);
    ...
    testFMULSU(0x40,0x00,0x00); //0,5*0 = 0
    printf("DataL: %X\n", output.outL);
    printf("DataH: %X\n", output.outH);
    assert(output.outL == 0x00);
